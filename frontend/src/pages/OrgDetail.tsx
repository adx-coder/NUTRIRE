...
export default function OrgDetail() {
  ...
  if (!org || !heroResult) return <NotFound onBack={() => navigate(-1)} />;

  const tone = reliabilityTone(org.reliability);
  const miles = (heroResult.distanceMeters / 1609.344).toFixed(1);
  const transit = bestTransit(org, heroResult.walkMinutes, heroResult.transitMinutes, heroResult.driveMinutes);
    ...
    <main className="relative min-h-screen overflow-hidden" style={{ background: "#EDE8E0" }}>
      ...
      <div className="relative z-10">
        ...
        <motion.nav initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}
          className="px-4 lg:px-6 pt-4 flex items-center justify-between">
        ...
        <div className="px-3 lg:px-5 pt-2 pb-4 lg:min-h-[calc(100dvh-56px)] grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] lg:grid-rows-[auto_auto_1fr] gap-2">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.1, ease }}
            className="lg:order-1 lg:row-span-3 rounded-2xl overflow-hidden bg-white/20 backdrop-blur-2xl border border-white/30 shadow-[0_4px_16px_rgba(0,0,0,0.05)] flex flex-col">
            ...
            <div className="px-4 py-3 flex items-center gap-2 border-t border-white/20">
              {tone.urgentCall ? (
              ...
              <button onClick={() => setShareOpen(true)} className="h-[38px] w-[38px] rounded-xl bg-white/30 border border-white/40 text-ink-muted flex items-center justify-center hover:bg-white/50 transition-colors shrink-0">
                <Share2 size={14} />
              </button>
          ...
          <GlassCard delay={0.15} className="lg:order-2">
            <div className="flex items-center gap-2 mb-1">
              <StatusDot state={heroResult.openStatus.state} />
              <span className="text-[12px] text-ink-soft font-medium">{heroResult.openStatus.label}</span>
            ...
            <p className="mt-1 text-[11px] text-ink-muted">{org.neighborhood || org.city || "DMV"}{org.state ? `, ${org.state}` : ""} · {transit.label}</p>
            {org.ai.heroCopy?.trim() && (
              <p className="mt-2.5 text-[13px] text-ink/70 leading-[1.5] text-pretty">{org.ai.heroCopy}</p>
            )}
          ...
          <GlassCard delay={0.25} className="lg:order-3">
            {/* What to expect */}
            {org.ai?.firstVisitGuide && org.ai.firstVisitGuide.length > 0 && (
              <div className="mb-3">
                <CardLabel>What to expect</CardLabel>
            ...
            <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.5, ease }}
              ...
              {org.ai.culturalNotes && <p className="mt-1.5 text-[10px] text-ink-muted italic">{org.ai.culturalNotes}</p>}
            </motion.div>

            {/* Hours */}
            <CardLabel>Hours</CardLabel>
          ...
          <GlassCard delay={0.35} className="lg:order-4">
            <div className="flex items-center justify-between mb-2">
              <CardLabel>If that doesn't work</CardLabel>
            ...
            <div className="mt-3 pt-2 border-t border-ink/5">
              {feedback === "thanks" ? <p className="text-[10px] text-ink-muted">Noted.</p> : (
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setFeedback("thanks")} className="h-6 w-6 rounded-full bg-white/40 border border-white/50 hover:bg-white/60 inline-flex items-center justify-center"><Check size={11} className="text-ink" /></button>
                  <button onClick={() => setFeedback("thanks")} className="h-6 w-6 rounded-full bg-white/40 border border-white/50 hover:bg-white/60 inline-flex items-center justify-center"><X size={11} className="text-ink" /></button>
                </div>
              )}
            </div>
          </GlassCard>
...
function GlassCard({ children, className, delay = 0 }: { children: ReactNode; className?: string; delay?: number }) {
  return (
    <motion.div initial={{ opacity: 0, y: 12, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.5, delay, ease }}
      ...
      className={clsx("rounded-2xl p-4 bg-white/30 backdrop-blur-2xl border border-white/40 shadow-[0_4px_16px_rgba(0,0,0,0.05),inset_0_1px_0_rgba(255,255,255,0.5)]", className)}>
      {children}
    </motion.div>
  );
}
...
function FormattedHours({ org }: { org: EnrichedOrganization }) {
  ...
  if (!p) return <p className="text-[12px] text-ink line-clamp-3">{org.hoursRaw || "Call for hours"}</p>;
  const days = DAYS.filter((d) => p[d] && p[d]!.length > 0);
  if (!days.length) return <p className="text-[12px] text-ink">{p.byAppointment ? "By appointment" : org.hoursRaw || "Call"}</p>;
...
function ShareSheet({ org, ranked, onClose }: { org: EnrichedOrganization; ranked: RankedOrg; onClose: () => void }) {
  const msg = [org.name, `${org.address}, ${org.city}, ${org.state}`, ranked.openStatus.label, org.ai.plainEligibility, org.phone ? `Call: ${org.phone}` : null, "— via Nutrire (NourishNet)"].filter(Boolean).join("\n");
