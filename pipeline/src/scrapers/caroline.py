...
ADDR_RE  = re.compile(
    ...
    r"(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Drive|Dr|Lane|Ln|Way|Court|Ct|Place|Pl|Circle|Cir|Pkwy)\.?",
    re.IGNORECASE,
)
...
NAV_RE = re.compile(r"^(about|contact|home|services|programs|resources|find food|volunteer|donate)$", re.I)


def scrape_caroline(content: str | bytes, source_id: str) -> list[RawRecord]:
    ...
    for heading in root.find_all(["h2", "h3", "h4"]):
        ...
        if not name or len(name) < 3 or len(name) > 150 or NAV_RE.match(name):
            continue
        ...
        while sib and depth < 10:
            ...
            if sib.name in ("ul", "ol"):
                lines.extend(li.get_text(strip=True) for li in sib.find_all("li"))
            else:
                lines.append(sib.get_text(" ", strip=True))
            sib = sib.find_next_sibling()
            depth += 1

        body = " ".join(filter(None, lines))
        ...
        requirements = [req for pattern, req in [
            ...
            (r"id required|photo id", "photo_id"),
        ] if re.search(pattern, raw_text, re.I)]

        services = ["food_pantry"]
        if re.search(r"hot meal|prepared meal", raw_text, re.I): services.append("hot_meals")
        if re.search(r"delivery", raw_text, re.I): services.append("delivery")

        food_types = [ft for pattern, ft in [
            (r"produce|vegetable|fruit", "produce"),
            (r"canned|non-perishable", "canned_goods"),
            (r"baby|infant", "baby_supplies"),
        ] if re.search(pattern, raw_text, re.I)]
        ...
        try:
            records.append(RawRecord(
                ...
                services=services, raw_text=raw_text,
            ))
        except Exception:
            pass
