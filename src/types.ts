...
export type LangCode = "en" | "es" | "am" | "vi" | "fr" | "zh" | "ar" | "ko" | "ru" | "ht" | "ur" | "bn" | "fa" | "fi" | "hi" | "pt" | "ti";
...
export interface TransitBusStop {
  stopId: string;
  stopName: string;
  route: string;
  allRoutes: string[];
...
export interface TransitDirections {
  metro?: { action: string; lines: string[]; station: string; walkMinutes: number | null };
  bus?: { action: string; routes: string[]; stopName: string; walkMinutes: number | null };
  recommended: "metro" | "bus";
  naturalDirections?: string;
}

export interface TransitEnrichment {
  nearestMetro: TransitStation | null;
  nearestBus: TransitBusStop | null;
  walkMinutesToMetro: number | null;
  walkMinutesToBus: number | null;
  reachableHoursOfWeek: number[];
  transitSummary: string;
  transitDirections: TransitDirections | null;
  enrichedAt: string;
}

// -------- weather alert (stage 9) --------

export interface WeatherAlert {
  event: string;
  level: "watch" | "warning" | "statement";
...
export interface AIEnrichment {
  ...
  model: string;
  /** Pre-translated AI text for non-English locales. */
...
export interface EnrichedOrganization {
  ...
  transit?: TransitEnrichment;
  nearestTransitType?: "metro" | "bus";
...
export interface OpenStatus {
  state: OpenState;
  /** User-facing label (English fallback). */
  label: string;
  /** i18n key for the label (e.g. "status.open", "status.opensAt"). */
  labelKey?: string;
  /** Interpolation vars for the label key. */
