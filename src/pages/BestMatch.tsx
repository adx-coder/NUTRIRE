...
import {
  ...
  Footprints, Bus, Car, Train, Check, Clock, AlertTriangle, CloudRain,
} from "lucide-react";
...
const METRO_COLORS: Record<string, string> = {
  RD: "#BF0D3E", BL: "#009CDE", OR: "#ED8B00", SV: "#A2A4A1", GR: "#00B140", YL: "#FFD100",
};
...
export default function BestMatch() {
  ...
  const heroStatusLabel = useStatusLabel(hero?.openStatus ?? { state: "unknown", label: "" });
  const weather = useWeather(hero?.org.lat, hero?.org.lon);
  const ai = useLocalizedAI(hero?.org.ai ?? { heroCopy: "", plainEligibility: "", firstVisitGuide: [], culturalNotes: null, toneScore: 0, qualityScore: 0, generatedAt: "", model: "" } as any);
  ...
  const shareOrg = async () => {
    const text = [org.name, `${org.address}, ${org.city}, ${org.state}`, hero.openStatus.label, ai.plainEligibility, org.phone ? `Call: ${org.phone}` : null, "via Nutrire (NourishNet)"].filter(Boolean).join("\n");
  ...
  return (
    ...
    >
      ...
      {/* ── NAV ── */}
      <motion.nav initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className="relative z-10 px-3 sm:px-5 lg:px-8 pt-3 sm:pt-4 pb-1 flex items-center justify-between">
        <button onClick={() => navigate("/")} className="h-10 px-3 rounded-lg text-[12px] font-medium text-ink-muted hover:text-ink hover:bg-ink/5 transition-colors inline-flex items-center gap-1.5">
          <ArrowLeft size={14} /> {t("nav.back")}
        </button>
        <span className="text-[11px] text-ink-muted font-medium truncate max-w-[140px] sm:max-w-none">{locationLabel}</span>
      </motion.nav>

      {/* ── BENTO GRID: map dominates left, cards stack right ── */}
      <div className="relative z-10 px-2 sm:px-3 lg:px-5 pt-2 sm:pt-3 pb-4 md:min-h-[calc(100dvh-72px)] grid grid-cols-1 md:grid-cols-[1fr_1fr] lg:grid-cols-[1.1fr_0.9fr] md:grid-rows-[auto_auto_1fr] gap-2 sm:gap-2.5">
        ...
        <motion.div
          ...
          className="md:order-2 rounded-2xl bg-white/30 backdrop-blur-2xl border border-white/40 shadow-[0_4px_16px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,0.5)] p-3 sm:p-4 flex flex-col gap-2 cursor-pointer"
          onClick={() => navigate(`/org/${org.id}`)}>
          <div>
            <div className="flex items-center gap-2 mb-1">
              <OpenDot state={hero.openStatus.state} />
              <span className="text-[12px] text-ink-soft font-medium">{heroStatusLabel}</span>
              <span className="text-ink/10">·</span>
              <span className="text-[12px] text-ink-muted tabular-nums">{miles} mi · {transit.label}</span>
              {transit.lines && transit.lines.length > 0 && (
                <span className="inline-flex items-center gap-0.5 ml-1">
                  {transit.lines.map((line) => (
                    <span key={line} className="h-3 min-w-[16px] px-1 rounded-full text-[8px] font-bold text-white flex items-center justify-center" style={{ backgroundColor: METRO_COLORS[line] || "#888" }}>{line}</span>
                  ))}
                </span>
              )}
            </div>
            ...
            {weather && (
              <div className={`mt-2 flex items-center gap-2 px-2.5 py-1.5 rounded-lg border ${weather.isAdverse ? "bg-mustard/10 border-mustard/15" : "bg-sage/5 border-sage/10"}`}>
        ...
        <motion.div
          ...
          className="md:order-3 rounded-2xl bg-white/30 backdrop-blur-2xl border border-white/40 shadow-[0_4px_16px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,0.5)] p-3 sm:p-4 flex flex-col">
          {org.ai?.firstVisitGuide && ai.firstVisitGuide.length > 0 && (
            <div>
              <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-ink-muted">{t("match.whatToExpect")}</p>
          ...
          {ai.culturalNotes && (
            <p className="mt-2 text-[10px] text-ink-muted italic">{ai.culturalNotes}</p>
          )}
        ...
        <motion.div
          ...
          className="md:order-1 md:row-span-3 rounded-2xl overflow-hidden bg-white/25 backdrop-blur-2xl border border-white/35 shadow-[0_4px_16px_rgba(0,0,0,0.06)] flex flex-col">
          <div className="flex-1 min-h-[200px] sm:min-h-[250px] md:min-h-[120px] relative">
            <MapGL
              initialViewState={{
          ...
          <div className="px-3 sm:px-4 py-2.5 flex items-center gap-2">
            ...
            <button onClick={shareOrg} aria-label="Share" className="h-[38px] w-[38px] rounded-xl bg-white/30 border border-white/40 text-ink-muted flex items-center justify-center hover:bg-ink/8 transition-colors shrink-0">
              <Share2 size={14} />
            </button>
            {org.phone && !tone.urgentCall && (
              <button onClick={callOrg} aria-label="Call" className="h-[38px] w-[38px] rounded-xl bg-white/30 border border-white/40 text-ink-muted flex items-center justify-center hover:bg-ink/8 transition-colors shrink-0">
                <Phone size={14} />
              </button>
            )}
          </div>
        ...
        <motion.div
          ...
          whileHover={{ scale: 1.005, transition: { duration: 0.2 } }}
          className="md:order-4 rounded-2xl bg-white/30 backdrop-blur-2xl border border-white/40 shadow-[0_4px_16px_rgba(0,0,0,0.06),inset_0_1px_0_rgba(255,255,255,0.5)] p-3 sm:p-4 flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-ink-muted">{t("match.alternatives")}</p>
  ...
  return (
    ...
    >
      <div className="flex gap-3 sm:gap-5">
        {/* Score ring */}
        <div className="relative shrink-0 w-[56px] h-[56px] sm:w-[72px] sm:h-[72px]">
      ...
      {/* Bottom data pills */}
      <div className="mt-3 flex flex-wrap items-center gap-1.5 sm:gap-2 text-[9px] text-ink-muted">
...
function bestTransit(org: RankedOrg["org"], walk: number, transit: number | null, drive: number): { Icon: LucideIcon; label: string; lines?: string[] } {
  if (walk <= 20) return { Icon: Footprints, label: `${walk} min walk` };
  // Use rich transit data if available
  if (org.transit?.transitSummary) {
    const Icon = org.nearestTransitType === "metro" ? Train : Bus;
    const lines = org.transit.nearestMetro?.lines;
    return { Icon, label: org.transit.transitSummary, lines };
  }
  if (org.nearestTransit) {
    const wk = org.nearestTransit.walkMinutes ?? Math.round(org.nearestTransit.distanceMeters / 80);
    return { Icon: org.nearestTransitType === "metro" ? Train : Bus, label: `${wk} min to ${org.nearestTransit.name.split(" (")[0]}` };
  }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6" style={{ background: "#EDE8E0" }}>
      <div className="max-w-[400px] text-center">
        <p className="font-display text-2xl font-semibold text-ink">{t("match.empty.title")}</p>
        <p className="mt-3 text-base text-ink-soft">
          {t("match.empty.body")} <Link to="/all" className="text-sage-deep underline font-medium">→</Link>
        </p>
      </div>
    </main>
  );
