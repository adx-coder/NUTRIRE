...
export default function Recommendations() {
  ...
  if (!gap) {
    return (
      <main className="min-h-screen flex items-center justify-center px-4" style={{ background: GLASS_BG }}>
        <GlassBackdrop />
        <div className="relative z-10 max-w-sm rounded-2xl bg-white/60 backdrop-blur-2xl border border-white/60 p-5 text-center shadow-[0_8px_32px_rgba(0,0,0,0.08)]">
          <p className="text-lg font-semibold text-ink">No recommendations are available yet.</p>
          <p className="mt-2 text-sm text-ink-soft">Try the map view or return home while the analysis data finishes loading.</p>
          <div className="mt-4 flex flex-wrap justify-center gap-2">
            <Link to="/map" className="inline-flex items-center justify-center h-10 px-4 rounded-xl bg-sage-deep text-white text-sm font-semibold hover:brightness-110 transition-all">
              Open map
            </Link>
            <button type="button" onClick={handleBack} className="inline-flex items-center justify-center h-10 px-4 rounded-xl bg-white/70 border border-white/70 text-sm font-medium text-ink hover:bg-white/90 transition-all">
              Go back
            </button>
          </div>
        </div>
    ...
    <main className="relative h-screen overflow-hidden" style={{ background: GLASS_BG }}>
      ...
      <div className="relative z-10 h-full flex flex-col">
        <motion.nav
          ...
          className="px-3 sm:px-5 lg:px-8 pt-4 sm:pt-5 flex items-center justify-between shrink-0"
        >
          <button
            type="button"
            onClick={handleBack}
            className="h-10 px-3 rounded-lg bg-white/40 backdrop-blur-xl border border-white/50 text-[12px] font-medium text-ink-muted hover:text-ink transition-colors inline-flex items-center gap-1.5 shadow-sm"
          >
            <ArrowLeft size={14} /> {t("nav.back")}
          </button>
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-ink-muted hidden sm:block">{t("rec.areasAnalyzed", { count: total })}</span>
            <button type="button" onClick={downloadInsights} className="h-10 px-3 rounded-lg bg-sage-deep text-white text-[12px] font-semibold inline-flex items-center gap-1.5 hover:brightness-110 shadow-sm">
              <Download size={13} /> JSON
            </button>
          </div>
        </motion.nav>

        <div className="flex-1 flex items-end md:items-center px-3 sm:px-5 lg:px-10 pb-4 sm:pb-6 md:pb-0 overflow-y-auto">
          <div className="w-full max-w-[520px]">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease }}>
              <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-sage-deep/50">{t("rec.eyebrow")}</p>
              <h1 className="mt-1 font-display text-[18px] sm:text-[20px] lg:text-[24px] font-bold text-ink tracking-tight leading-tight">
                {t("rec.title")}
              </h1>
            </motion.div>
            ...
            <motion.div
              ...
              className="mt-3 sm:mt-4 rounded-2xl bg-white/50 backdrop-blur-2xl border border-white/60 shadow-[0_8px_32px_rgba(0,0,0,0.08),inset_0_1px_0_rgba(255,255,255,0.7)] p-3.5 sm:p-5 lg:p-6"
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={prev} className="h-8 w-8 rounded-full bg-white/50 border border-white/60 flex items-center justify-center hover:bg-white/70 transition-colors">
                    <ChevronLeft size={16} className="text-ink" />
                  </motion.button>
                  <span className="text-[12px] text-ink-muted tabular-nums font-medium">{current + 1} of {total}</span>
                  <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={next} className="h-8 w-8 rounded-full bg-white/50 border border-white/60 flex items-center justify-center hover:bg-white/70 transition-colors">
              ...
              <AnimatePresence mode="wait">
                <motion.div key={gap.zip} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.3, ease }}>
                  ...
                  <div className="mt-4 space-y-2.5">
                    <Bar label={t("rec.need")} value={gap.needScore} pct={needPct} color="#C96F4A" />
                    <Bar label={t("rec.supply")} value={gap.supplyScore} pct={supplyPct} color="#4F7F6A" />
                    <Bar label={t("rec.gap")} value={gap.gap} pct={gapPct} color="#D9A441" bold />
                  </div>

                  <div className="mt-4 flex flex-wrap items-center gap-2 sm:gap-3 text-[11px] text-ink-muted">
                    <span><span className="font-bold text-[14px] text-ink">{(gap.population / 1000).toFixed(0)}K</span> {t("rec.people")}</span>
                    <span className="text-ink/10">-</span>
                    <span><span className="font-bold text-[14px] text-ink">{gap.underservedPopulation.toLocaleString()}</span> {t("rec.underserved")}</span>
                    <span className="text-ink/10">-</span>
                    <span><span className="font-bold text-[14px] text-ink">{gap.nearbyOrgCount}</span> {t("rec.orgs")}</span>
                  </div>
            ...
            <div className="mt-3 flex items-center justify-center gap-0.5">
              {gaps.slice(0, Math.min(20, total)).map((_, i) => (
                <button key={i} type="button" onClick={() => goTo(i)} aria-label={`Go to area ${i + 1}`} className="p-1">
                  <span className={`block rounded-full transition-all duration-200 ${i === current ? "w-5 h-1.5 bg-terracotta" : "w-1.5 h-1.5 bg-ink/15 hover:bg-ink/30"}`} />
                </button>
              ))}
  ...
  return (
    <div>
      ...
      <div className={`${bold ? "h-2" : "h-1.5"} rounded-full bg-ink/6 overflow-hidden`}>
        <motion.div className="h-full rounded-full" style={{ backgroundColor: color }} initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.8, delay: 0.2, ease: [0.22, 1, 0.36, 1] }} />
      </div>
    </div>
  );
}
