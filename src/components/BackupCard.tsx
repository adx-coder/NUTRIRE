...
export function BackupCard({ result, onClick, className }: Props) {
  ...
  const statusLabel = useStatusLabel(openStatus);
  ...
  return (
    <motion.button
      ...
      aria-label={`${org.name}, ${statusLabel}`}
    >
      <div className="relative px-4 py-3.5">
        ...
        <div className="mt-1 flex items-center gap-2">
          ...
          <span className="text-[11px] text-ink-soft font-medium">{statusLabel}</span>
        </div>

        {ai.heroCopy?.trim() && (
          <p className="mt-2 text-[12px] text-ink/60 leading-snug line-clamp-2">{ai.heroCopy}</p>
        )}

        <div className="mt-2 flex items-center gap-1.5 flex-wrap">
          <p className="text-[11px] text-sage-deep font-medium truncate flex-1">{ai.plainEligibility}</p>
          {org.nearestTransitLines && org.nearestTransitLines.length > 0 && (
            <span className="px-1.5 py-0.5 rounded bg-ink/5 text-[9px] font-semibold text-ink/40 shrink-0">{org.nearestTransitLines[0]}</span>
          )}
...
function pickMode(org: RankedOrg["org"], walk: number, transit: number | null, drive: number): string {
  ...
  if (org.nearestTransit) {
    ...
    return `${wk}m to ${org.nearestTransit.name.split(" (")[0]}`;
  }
